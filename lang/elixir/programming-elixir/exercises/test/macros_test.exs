defmodule Exercises.Macros.Test do
  ...
  describe "when using unless macro" do
    test "it must execute :do clause" do
      assert :do == exprt(Macros.unless(false, do: :do, else: :else))
    end

    test "it must execute :else clause" do
      assert :else == exprt(Macros.unless(true, do: :do, else: :else))
    end
  end
  ...
  describe "when using explain macro" do
    test "it must handle simple sum" do
      assert "sum 2 and 1" == exprt(Macros.explain(do: 2 + 1))
    end

    test "it must handle simple subtraction" do
      assert "subtract 1 from 2" == exprt(Macros.explain(do: 2 - 1))
    end

    test "it must handle simple multiplication" do
      assert "multiply 5 and 2" == exprt(Macros.explain(do: 5 * 2))
    end

    test "it must handle simple division" do
      assert "divide 10 by 2" == exprt(Macros.explain(do: 10 / 2))
    end

    test "it must handle position priority" do
      assert "multiply 4 and 10, then divide by 2" == exprt(Macros.explain(do: 4 * 10 / 2))
    end

    test "it must handle multiplication priority" do
      assert "multiply 3 and 4, then add 2" == exprt(Macros.explain(do: 2 + 3 * 4))
    end

    test "it must handle division priority" do
      assert "divide 4 by 2, then add 2" == exprt(Macros.explain(do: 2 + 4 / 2))
    end

    test "it must handle parenthesis priority" do
      assert "sum 2 and 1, then multiply by 3" == exprt(Macros.explain(do: 3 * (2 + 1)))
    end
